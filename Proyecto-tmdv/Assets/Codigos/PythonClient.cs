...
public class PythonClient : MonoBehaviour
{
    ...
    void MoveDrone(GameObject drone, List<Vector2> path, ref int index)
    {
        if (path.Count > 0 && index < path.Count)
        {
            ...
            // Mover en el plano X-Z en lugar de X-Y
            Vector3 newPosition = new Vector3(target.x, drone.transform.position.y, target.y);
            
            drone.transform.position = Vector3.MoveTowards(drone.transform.position, newPosition, Time.deltaTime * 5);
            
            if (Vector2.Distance(new Vector2(drone.transform.position.x, drone.transform.position.z), target) < 0.1f)
                index++;
        }
    }
}
